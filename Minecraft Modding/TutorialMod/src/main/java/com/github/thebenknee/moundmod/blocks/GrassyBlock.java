...
public class GrassyBlock extends GrassBlock
{

	private String dirt;
    public GrassyBlock(Properties properties) {
        super(properties);
        this.dirt = dirt;
    }
    ...
    public void tick(BlockState state, World worldIn, BlockPos pos, Random random) {
        if (!worldIn.isRemote) {
            ...
            if (!getLightLevel(state, worldIn, pos)) {
                worldIn.setBlockState(pos, MoundBlocks.goblin_grass.getDefaultState());
                ...
                if (worldIn.getLight(pos.up()) >= 0) {
                    ...
                    for (int i = 0; i < 4; ++i) {
                        ...
                        if (worldIn.getBlockState(blockpos).getBlock() == MoundBlocks.goblin_dirt && getFluidState(blockstate, worldIn, blockpos)) {
